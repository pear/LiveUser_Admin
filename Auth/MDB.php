...
class LiveUser_Admin_Auth_MDB extends LiveUser_Admin_Auth_Common
{
     ...
     * @return void
     */
    function LiveUser_Admin_Auth_MDB(&$connectOptions, $name = null)
    {
    ...
    function removeUser($authId)
    {
        ...
        $query = '
            DELETE FROM
                ' . $this->authTable . '
            WHERE
                '.$this->authTableCols['required']['auth_user_id']['name'].'=' . $this->dbc->getValue($this->authTableCols['required']['auth_user_id']['type'], $authId);
